...
def med_reshape(image, new_shape):
    ...
    x = np.minimum(image.shape[0],reshaped_image.shape[0]) -1
    y = np.minimum(image.shape[1],reshaped_image.shape[1]) -1
    z = np.minimum(image.shape[2],reshaped_image.shape[2]) -1
